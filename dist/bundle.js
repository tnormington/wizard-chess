...
var View = {
  ...
  renderUnitMenu: function renderUnitMenu(unit, tileIndex) {
    if (unit.abilities) {
      var abilities = unit.abilities.map(function (ability) {
        return '<li \n          class="unit-menu__option ability ' + ability.name.toLowerCase().replace(' ', '-') + '"\n          data-ability-function="' + ability.function + '">' + ability.name + '</li>';
      }).join('');
      var list = this.stringToNode('<ul data-unit-id="' + unit.id + '" data-tile-index="' + tileIndex + '">' + abilities + '</ul>');
      this.clearUnitMenu();
  ...
  clearUnitMenu: function clearUnitMenu() {
  ...
  setupHandlers: function setupHandlers() {
    window.handlers = {
      ...
      drawCard: function drawCard() {
        _Model2.default.Hand.drawRandomCard();
      },
      endTurn: function endTurn() {
        _Controller2.default.endTurn();
      },
      closeUnitMenu: function closeUnitMenu() {
        View.clearUnitMenu();
      }
    };
    ...
    var gridContainer = document.getElementById('grid-container');
    gridContainer.addEventListener('click', _Controller2.default.handleTileClick);
    ...
    this.unitMenu.addEventListener('click', _Controller2.default.handleUnitMenuClick);
...
var _postal = __webpack_require__(3);
...
var Controller = {
  ...
  createGrid: function createGrid() {
    _Model2.default.Grid.clearGridArray();
    _View2.default.removeAllTileMarkup();
    for (var rowIndex = 0; rowIndex < _Model2.default.Grid.rows; rowIndex++) {
      // loop over each row
      var row = [];
      for (var columnIndex = 0; columnIndex < _Model2.default.Grid.columns; columnIndex++) {
      ...
      _Model2.default.Grid.data.push(row);
    }
    // console.table(grid);
    this.renderGrid();
  },
  ...
  updateTile: function updateTile(newTile) {
    ...
    newTile.tileIndex = _Model2.default.Grid.data[row][column].tileIndex;

    // console.log('TEST: ', newTile.tileIndex === grid[row][column].tileIndex);
    _Model2.default.Grid.data[row][column] = newTile;
    this.renderGrid();
  ...
  placeRandomUnit: function placeRandomUnit() {
    var randomRow = Math.floor(Math.random() * _Model2.default.Grid.rows);
    var randomColumn = Math.floor(Math.random() * _Model2.default.Grid.columns);

    var tileIndex = String('' + randomRow + randomColumn);

    // console.log('random tileIndex: ', tileIndex);

    var randomUnit = _Model2.default.Units.getRandomUnit(tileIndex);
  ...
  renderGrid: function renderGrid() {
    var gridMarkup = _Model2.default.Grid.data.map(function (row) {
      // loop over each row
  ...
  handleTileClick: function handleTileClick(e) {
    ...
    var unit = _Model2.default.Units.getUnitById(e.target.dataset.unitId);
    ...
    _View2.default.renderUnitMenu(unit, tileIndex);
  ...
  handleUnitMenuClick: function handleUnitMenuClick(e) {
    if (!e.target.classList.contains('unit-menu__option')) return;
    var unit = _Model2.default.Units.getUnitById(e.target.parentNode.dataset.unitId);
    var abilityFunction = e.target.dataset.abilityFunction;

    var tileIndex = e.target.parentNode.dataset.tileIndex;
    ...
    var fn = _Model2.default.Units[abilityFunction];
  ...
  handleGridDrop: function handleGridDrop(e) {
    ...
    _Model2.default.Hand.removeCardFromHand(card);

    // update grid with new tile
    var unit = _Model2.default.Units.getUnitById(id);
...
var Units = {
  ...
  basicAttack: function basicAttack(unit) {
...
( function( root, factory ) {
	...
	if ( true ) {
		...
		!(__WEBPACK_AMD_DEFINE_ARRAY__ = [ __webpack_require__(9) ], __WEBPACK_AMD_DEFINE_RESULT__ = function( _ ) {
...
var Hand = {
  ...
  drawCard: function drawCard(card) {
    // if the handCount is at max, break
    if (this.handCount >= this.max) return;
    console.log(card);
    var cardInHand = this.addUnitToHand(card);
  ...
  drawRandomCard: function drawRandomCard() {
    var randomUnit = _Units2.default.getRandomUnit();

    this.drawCard(randomUnit);
  },

  addUnitToHand: function addUnitToHand(unit) {
    var card = '<div class="hand__card" id="' + (unit.content ? unit.content : '') + '" draggable="true">' + unit.name + '\n        <span class="hand__card__cost">\n          ' + unit.cost + '\n        </span>\n        ' + (unit.health ? '<span class="hand__card__health">' + unit.health + '</span>' : '') + '\n        ' + (unit.damage ? '<span class="hand__card__damage">' + unit.damage + '</span>' : '') + '\n      </div>';
...
var postal = __webpack_require__(3);
...
function init() {
  ...
  // draw 5 random cards
  for (var x = 0; x < 5; x++) {
    _Hand2.default.drawRandomCard();
  }

  _Socket2.default.setupSocketHandlers();
  (0, _Subscriptions2.default)();
  // States.beginTurn();
}
...
function setupSubscriptions() {
    ...
    var socketMessage = postal.subscribe({
        channel: "socket",
        topic: "message.send",
    ...
    var socketOpen = postal.subscribe({
        channel: "socket",
        topic: "status.open",
        callback: function callback(data, envelope) {
            // `data` is the data published by the publisher. 
    ...
    var socketClose = postal.subscribe({
        channel: "socket",
        topic: "status.close",
        callback: function callback(data, envelope) {
            // `data` is the data published by the publisher. 
    ...
    var socketError = postal.subscribe({
        channel: "socket",
        topic: "error",
        callback: function callback(data, envelope) {
            // `data` is the data published by the publisher. 
    ...
    var playerPlayedCard = postal.subscribe({
        channel: "playerAction",
        topic: "hand.playedCard",
        callback: function callback(data, envelope) {
    ...
    var playerHoverTile = postal.subscribe({
        channel: "playerAction",
        topic: "hand.hoverTile",
        callback: function callback(data, envelope) {
            // console.log(data.message);
    ...
    var playerActionGridUpdateTile = postal.subscribe({
        channel: "playerAction",
        topic: "grid.updateTile",
        callback: function callback(data, envelope) {
            // console.log(data.message);
...
exports.default = setupSubscriptions;
